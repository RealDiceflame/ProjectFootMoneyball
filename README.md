...
The injury section reads the existing saved `player_news.json`, not a visitor-triggered provider API. It displays report weeks separately from the snapshot's save time, warns about stale/unavailable data, and does not infer health from missing coverage. Questionable/probable or practice participation alone do not trigger its Risk badge. Product-update cards and highlight selections in `docs/index.html` are manually curated and dated, not automatic breaking news. Highlight links go to the official source; no video is rehosted. The official NFL X timeline loads automatically and asynchronously on the homepage, with reduced personalization requested and a visible third-party privacy notice. Blocking or login requirements leave direct-source links available. Instagram integration is not enabled. This public homepage does not introduce accounts or shared leagues.
