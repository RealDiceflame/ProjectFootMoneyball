...
The homepage opens with “Be an Outlier. Know your Baseline.” and a weekly reported-score strip. Cards show team logos and link to game box scores; the default week changes 24 hours before the next week's first kickoff. Scores refresh independently of rankings, about every 15 minutes near games and every six hours otherwise. They are not a live game-clock feed. See [scoreboard sources, scheduling and safeguards](documentation/scoreboard.md) and [the saved team/player game-stat archive](documentation/game-stats.md).

The injury section reads the existing saved `player_news.json`, not a visitor-triggered provider API. It displays report weeks separately from the snapshot's save time, warns about stale/unavailable data, and does not infer health from missing coverage. Questionable/probable or practice participation alone do not trigger its Risk badge. Product-update cards in `docs/index.html` are manually curated and dated. Around the league uses saved ESPN NFL RSS headlines with source links and matched player photos, without injury badges; no video is rehosted. The official NFL X timeline loads asynchronously with reduced personalization requested. Source details and the third-party privacy notice are grouped in the expandable homepage footer. Blocking or login requirements leave direct-source links available. Instagram integration is not enabled. This public homepage does not introduce accounts or shared leagues.

## Use the web draft board

Open <https://outlierbaseline.com/rankings.html> in any modern browser. The website supports every team-count, QB, PPR, and TE-premium ranking combination, persistent drafted-player markers, column filters, sorting, CSV export, rookie and current-injury labels, source-linked player intel, and up to ten completed seasons of year-by-year player stats. Click a player name to see current roster status, depth position, material arrivals/departures, injury data, matched ESPN RSS headlines, format-aware scoring charts, an age-adjusted season projection, and historical performance. The Projection Lab plots every qualifying player-season by age and position, overlays the selected player, summarizes positional scoring standard deviation, and maps expected season points by position and ADP round. Age Curve v1 uses the full 2016–2025 history for position aging and uncertainty, while weighting each player's latest three seasons 5:3:2 so old production does not overpower recent form. It learns bounded position-specific age changes from consecutive seasons, estimates games from recent availability, and displays a one-standard-deviation range. Historical fantasy points recalculate for the selected PPR and TE-premium settings. Public ADP is limited to the supported Sleeper and MyFantasyLeague feeds plus the manually maintained Yahoo snapshot. Every available provider value remains visible on every board; only a provider that lacks a player is blank. Sources and ADP SD show evidence count and standard deviation. A filterable 0–100 volatility index combines season-to-season scoring variation (70%) with relative ADP disagreement (30%), using only the available component when the other lacks enough evidence. Imported data stays in that browser, works across league settings, and is never uploaded. Separate Kicker & D/ST and Weekly Odds pages cover special-teams ADP and upcoming NFL game markets. Desktop downloads remain available under GitHub Releases.
...
`league.html` runs 2,000–10,000 seasons in a browser worker. It shows each team's game-by-game score average, SD and middle-80% range; expected season W–L–T and scoring totals; division, playoff, bye, conference and Super Bowl probabilities. Every run uses fresh random draws by default; enable **Use a fixed seed** to reproduce draws with the same snapshot and game status. It retains 20 complete examples spread through the run, each with its own 272 regular-season game results, final records, playoff seeds and 13-game postseason bracket. Browsing examples does not change aggregate probabilities: those use every trial. Completed regular-season scores stay fixed. Started games lacking final scores are explicitly unresolved, not live forecasts. Each remaining matchup has one shared sampled score for both opponents. Interrupted or incomplete worker runs are reported as failures, not shown as complete results. Similar aggregate probabilities or the same champion across runs are expected possibilities, not proof that a simulation is stuck.
